(function(){
  ...
  /* PRESTANDAVALET ÄR BORTTAGET (2026-09-10). Det låg först i varje widgets panel, flyttade till
     Inställningar i checkpoint 49 — och togs sedan bort helt på Davids beslut: konkurrenten har
     inget sådant val, och tre lägen som ingen ställer in är tre lägen som bara kan bli fel.

     LÄGET LÅSES TILL standard, och den sparade nyckeln rensas. Utan den raden hade den som en gång
     valde "Låg" suttit fast i ett läge som släcker skuggor och partiklar, utan något sätt att ta sig
     ur det. runtime-controls.css behåller sina [data-performance]-regler — de är ofarliga när inget
     matchar, och sparar arbetet om läget någon gång behövs igen. */
  if(localStorage.getItem(PERF_KEY)&&localStorage.getItem(PERF_KEY)!=='standard'){
    localStorage.removeItem(PERF_KEY);mode='standard';applyPerformance();
  }
  const oldBind=bind;
