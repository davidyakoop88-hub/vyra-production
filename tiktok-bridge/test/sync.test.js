...
test('starts a bridge for a connection registered after boot', async () => {
  ...
  assert.deepEqual(result, { started: 0, stopped: 0, refused: 0, restarted: 0, running: 0, waiting: 0 }, 'empty table starts nothing');

  h.setRows([{ workspace_id: 'ws-1', tiktok_username: 'piiikabom' }]);
  result = await h.manager.syncOnce();

  assert.deepEqual(result, { started: 1, stopped: 0, refused: 0, restarted: 0, running: 1, waiting: 0 });
...
test('stops the bridge when the row goes inactive', async () => {
  ...
  assert.deepEqual(result, { started: 0, stopped: 1, refused: 0, restarted: 0, running: 0, waiting: 0 });
...
test('restarts the bridge when the username changes', async () => {
  ...
  assert.deepEqual(result, { started: 1, stopped: 1, refused: 0, restarted: 0, running: 1, waiting: 0 });
...
test('is idempotent — an unchanged table spawns nothing new', async () => {
  ...
  assert.deepEqual(result, { started: 0, stopped: 0, refused: 0, restarted: 0, running: 1, waiting: 0 });
...
test('handles several workspaces at once', async () => {
  ...
  assert.deepEqual(result, { started: 1, stopped: 1, refused: 0, restarted: 0, running: 2, waiting: 0 });
