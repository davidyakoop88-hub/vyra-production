...
// viewer_levels lagrar en identifierare per TikTok-tittare. De personerna har aldrig ingatt
// nagot avtal med oss, och tabellen vaxte tidigare utan bortre grans. 180 dagar ar samma
// horisont som audit_log och racker med god marginal for fanklubbsnivaer, som ror sig pa veckor.
async function cleanupAuthData(){try{const sessions=await pool.query("DELETE FROM sessions WHERE expires_at<now() OR (mfa_verified_at IS NULL AND created_at<now()-interval '30 minutes')"),tokens=await pool.query("DELETE FROM auth_tokens WHERE expires_at<now()-interval '7 days' OR consumed_at<now()-interval '7 days'"),audit=await pool.query("DELETE FROM audit_log WHERE created_at<now()-interval '180 days'"),viewers=await pool.query("DELETE FROM viewer_levels WHERE seen_at<now()-interval '180 days'");if(sessions.rowCount||tokens.rowCount||audit.rowCount||viewers.rowCount)console.log(JSON.stringify({level:'info',event:'auth_cleanup',sessions:sessions.rowCount,tokens:tokens.rowCount,audit:audit.rowCount,viewers:viewers.rowCount,at:new Date().toISOString()}))}catch(error){console.error(JSON.stringify({level:'error',event:'auth_cleanup_failed',message:error.message,at:new Date().toISOString()}))}}
