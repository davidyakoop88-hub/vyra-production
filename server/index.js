...
const publicAccess=p.match(/^\/api\/overlay-access\/([^/]+)(?:\/(.*))?$/);if(publicAccess){const access=await overlayAccess(publicAccess[1]);if(!access)return send(res,401,{ok:false,error:'Overlay-länken är ogiltig eller har gått ut'});if(req.method!=='GET')return send(res,405,{ok:false,error:'Metoden stöds inte'});const rest=publicAccess[2]||'';
  ...
  if(p==='/api/auth/mfa/setup'&&req.method==='POST'){if(s.mfa_enabled_at)return send(res,409,{ok:false,error:'MFA är redan aktiverat'});const secret=MFA.newSecret();await pool.query("UPDATE users SET mfa_secret_enc=$1,mfa_recovery_hashes='[]'::jsonb WHERE id=$2",[MFA.encryptedSecret(secret),s.user_id]);return send(res,200,{ok:true,secret,uri:MFA.uri(secret,s.email)})}
