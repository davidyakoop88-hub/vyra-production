...
async function ingestTikTokEvent(workspaceId,payload){
  ...
  // GAVOKATALOGEN fylls passivt av varje gava som passerar. Samma monster och samma skal som
  // raderna ovan: inte await:at, svaljer sina egna fel, bara pa !duplicate.
  //
  // Det har ar den kalla som tacker det TikToks egen lista INTE kan ge: exklusiva gavor finns bara
  // i sitt eget rum, och listan svarar is_full_gift_data:false. Unionen over alla anslutna rum blir
  // darfor storre an vad nagot enskilt konto ser.
  if(!raw.duplicate)Gavokatalog.noteraFranEvent(pool,raw.event).catch(()=>{});
...
async function openEventStream(req,res,u,workspaceId,accessToken=null,overlayId=null){
  ...
  const write=item=>{const chunk=GoalSse.sseChunk(item,overlayId);if(chunk)res.write(chunk)};
...
const publicAccess=p.match(/^\/api\/overlay-access\/([^/]+)(?:\/(.*))?$/);if(publicAccess){const access=await overlayAccess(publicAccess[1]);if(!access)return send(res,401,{ok:false,error:'Overlay-länken är ogiltig eller har gått ut'});if(req.method!=='GET')return send(res,405,{ok:false,error:'Metoden stöds inte'});const rest=publicAccess[2]||'';
  ...
  if(p==='/api/admin/gavokatalog'&&req.method==='POST'){const d=await body(req,8*1024*1024);const ut=await Gavokatalog.noteraKatalog(pool,d&&d.gifts);return send(res,200,{ok:true,...ut})}
  // Rakneverk, aldrig id:n. Svaret sager HUR MANGA som finns, inte VILKA.
  if(p==='/api/admin/gavokatalog/status'&&req.method==='GET'){const k=await pool.query("SELECT kalla,count(*)::int n FROM gavokatalog GROUP BY kalla");const r=await pool.query("SELECT rule_key,status,count(*)::int n FROM gavoregel GROUP BY rule_key,status");return send(res,200,{ok:true,katalog:k.rows,regler:r.rows})}
  ...
  if(verifieraRegel&&req.method==='POST'){const ruleKey=Regelnycklar.validera(decodeURIComponent(verifieraRegel[1]));if(!ruleKey)return send(res,400,{ok:false,error:'Okänd regelnyckel'});const d=await body(req);const ut=await Gavokatalog.verifiera(pool,ruleKey,String(d&&d.giftId||''));return ut.ok?send(res,200,{ok:true}):send(res,409,{ok:false,skal:ut.skal})}
  // ATERKALLA. Statusen blir 'inaktiverad', inte raderad: historiken om vad som en gang godkandes
  // ar sjalva poangen med en mansklig grind. Posten slutar matcha i samma andetag.
  const inaktiveraRegel=p.match(/^\/api\/admin\/gavoregel\/([A-Za-z0-9_:-]{1,160})\/inaktivera$/i);
  if(inaktiveraRegel&&req.method==='POST'){const ruleKey=Regelnycklar.validera(decodeURIComponent(inaktiveraRegel[1]));if(!ruleKey)return send(res,400,{ok:false,error:'Okänd regelnyckel'});const d=await body(req);const ut=await Gavokatalog.inaktivera(pool,ruleKey,String(d&&d.giftId||''));return ut.ok?send(res,200,{ok:true}):send(res,409,{ok:false,skal:ut.skal})}
  // KANDIDATLISTAN. Enda stallet dar id:n lamnar servern — utan den kan ingen manniska se vad som
  ...
  return send(res,404,{ok:false,error:'Adminfunktionen saknas'})}
  ...
  // Studio's own goal stream. The workspace stream below stays exactly as it is — raw events only,
  // no frames — because a frame belongs to one overlay and a workspace-level connection cannot say
  // which. This route can: the overlay is named in the path, the caller is a signed-in member of the
  // workspace, and the id is then confirmed to BE in that workspace before anything is opened. The
  // scope handed to openEventStream is the row's own id, from the database, so a valid-looking id
  // that belongs somewhere else never becomes a subscription — it is a 404.
  //
  // Same filter, same frames: GoalSse.sseChunk() is what writes, exactly as it does for an OBS link.
  const overlayStream=p.match(/^\/api\/workspaces\/([0-9a-f-]+)\/overlays\/([0-9a-f-]+)\/events\/stream$/i);
  if(overlayStream){const[,workspaceId,overlayId]=overlayStream;
    ...
    const scoped=await pool.query('SELECT id FROM overlays WHERE id=$1 AND workspace_id=$2',[overlayId,workspaceId]);
  ...
  // real follower goal up with fake events and have no way to tell which part of the number was
  // real. Nothing here writes to Postgres — no applyEvent, no claim, no frame — and
  // studio-goal-stream.test.js fails if that ever changes.
  ...
  // ADMINISTRATIV ATEROPPNING AV ETT STANGT RUM. Ligger efter den gemensamma sessionsraden och
  // arver darfor husets bada skydd: den globala sameOrigin-sparren (403 'Origin nekad' fore allt
  // annat) och CSRF-kravet pa icke-GET (session() returnerar null utan giltig x-vyra-csrf -> 401).
  // Flaggan kontrolleras forst i hanteraren: fail-closed, 503 och noll skrivningar.
  // Aktoren ar sessionens anvandare - bodyn tillfragas aldrig om vem som agerar.
  const reopenRoute=p.match(/^\/api\/workspaces\/([0-9a-f-]+)\/stream-rooms\/([^/]+)\/reopen$/i);
  if(reopenRoute&&req.method==='POST'){
    if(process.env.VYRA_SANDNINGSIDENTITET!=='1')return send(res,503,{ok:false,error:'Sändningsidentiteten är inte aktiverad'});
    const workspaceId=reopenRoute[1];
    if(!await membership(s.user_id,workspaceId,['owner','admin']))return send(res,403,{ok:false,error:'Behörighet saknas'});
    const d=await body(req);
    const ut=await StreamSessions.tillatRumIgen({workspaceId,roomId:reopenRoute[2],actorUserId:s.user_id,skal:d?.reason});
  ...
  // The goal API for a signed-in member. Both ids come from the URL, and neither is trusted on its
  // own: the workspace decides whether this user may be here at all, and the overlay is looked up
  ...
  if(goalRoute){const[,workspaceId,overlayId,rawWidgetId,isReset]=goalRoute;
    ...
    if(!await membership(s.user_id,workspaceId,roles))return send(res,403,{ok:false,error:'Behörighet saknas'});
    let widgetId=null;
    if(rawWidgetId!==undefined){try{widgetId=decodeURIComponent(rawWidgetId)}catch{return send(res,400,{ok:false,error:'Ogiltigt widget-id'})}}
    const overlayMissing=()=>send(res,404,{ok:false,error:'Overlay saknas'});
    const widgetMissing=()=>send(res,404,{ok:false,error:'Målwidgeten finns inte i den här overlayn'});
    if(req.method==='GET'&&!widgetId){const out=await GoalRuntime.listGoals(pool,overlayId,{workspaceId});return out.missing?overlayMissing():send(res,200,{ok:true,goals:out.goals})}
    if(req.method==='PATCH'&&widgetId&&!isReset){const d=await body(req,4096),check=GoalRuntime.validateGoalPatch(d);if(check.error)return send(res,400,{ok:false,error:check.error});
      const out=await GoalRuntime.patchGoal(pool,overlayId,widgetId,check.patch,{workspaceId});
      if(out.missing)return overlayMissing();if(out.unknownWidget)return widgetMissing();
      return send(res,200,{ok:true,goal:out.goal})}
    if(req.method==='POST'&&widgetId&&isReset){const out=await GoalRuntime.resetGoalWidget(pool,overlayId,widgetId,{workspaceId});
      if(out.missing)return overlayMissing();if(out.unknownWidget)return widgetMissing();
      return send(res,200,{ok:true,goal:out.goal})}
