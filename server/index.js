...
const {release:desktopRelease,fromBrowser:desktopFromBrowser}=require('./desktop-release');
...
const publicAccess=p.match(/^\/api\/overlay-access\/([^/]+)(?:\/(.*))?$/);if(publicAccess){const access=await overlayAccess(publicAccess[1]);if(!access)return send(res,401,{ok:false,error:'Overlay-länken är ogiltig eller har gått ut'});if(req.method!=='GET')return send(res,405,{ok:false,error:'Metoden stöds inte'});
  ...
  if(p==='/api/billing/webhook'&&req.method==='POST'){try{const result=await Billing.webhook(pool,await rawBody(req,1024*1024),req.headers);/* Framgang loggas ocksa — annars syns bara avslagen, och det skarpa PayPal-kopet (docs/lansering.md §1) gar inte att folja i Railway-loggen. */console.log(JSON.stringify({level:'info',event:'billing_webhook_ok',type:result.type,duplicate:!!result.duplicate,at:new Date().toISOString()}));return send(res,200,{ok:true,...result})}catch(error){console.error(JSON.stringify({level:'warn',event:'billing_webhook_rejected',message:error.message,at:new Date().toISOString()}));return send(res,400,{ok:false,error:'Ogiltig webhook'})}}
