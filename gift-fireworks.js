...
function fwSequence(w,combo){const level=fwLevelOf(combo);if(w.fwTheme==='supernova')return{level,duration:Number(combo)>=100?18:Number(combo)>=10?9:6,impact:1.6,flight:1.4};return {level,duration:fwDurationOf(w)+level.lead,impact:fwFlightOf(w)+level.lead,flight:fwFlightOf(w)}}
...
const oldWh=wh;wh=function(w){if(w.type!=='templateGiftFireworks')return oldWh(w);return `<div class="widget templateGiftFireworks${selected===w.id?' selected':''}" data-id="${w.id}" style="left:${w.x}px;top:${w.y}px;width:${w.width||360}px${fwThemeOf(w)==='supernova'?';height:'+((w.width||540)*800/960)+'px!important':''}"><div class="gift-fireworks-fx" data-fw-theme="${fwThemeOf(w)}" style="--duration:${fwDurationOf(w)}s;--gift:${w.fwGiftSize||110}px"></div><span class="resize-handle">↘</span></div>`};
...
function fwSpela(w,e,combo,gavobild,avatarbild){
 ...
 if(theme==='supernova'){e.style.animation='none';group.style.animation='none';group.style.translate='none';e.append(group);e.classList.toggle('play',true);window.VyraSupernova.mount(group,w,combo,gavobild,avatarbild);fwSchedule(e);return}
...
function fwRenderPreview(e,w){
 if(!e)return;
 if(w.fwTheme==='supernova'){window.VyraSupernova?.preview(e,w);return}
...
document.addEventListener('click',event=>{if(!event.target.closest('[data-fw] button'))return;setTimeout(()=>{let w=state.widgets.filter(x=>x.type==='templateGiftFireworks').at(-1);if(!w||w.fwTheme==='supernova')return;w.x=120;w.y=380;w.width=360;w.hidden=false;w.fwGiftImage=campaignGiftList()[0]?.file;selected=w.id;save();render()},0)});
