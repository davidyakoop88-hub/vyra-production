...
function fwSpela(w,e,combo,gavobild,avatarbild,avsandare,gavonamn){
  const gift=VyraSafe.src(gavobild,VyraSafe.src(w.fwGiftImage,FW_GIFT));
  buildComboRockets(w,e,combo,gift);
  const mitt=[...e.children].find(n=>n.tagName==='IMG');
  if(mitt){mitt.src=gift;mitt.dataset.fallbackApplied='1';mitt.onerror=()=>{mitt.onerror=null;mitt.src=FW_GIFT}}
  const avatar=e.querySelector('.fw-sender-avatar');
  const bild=VyraSafe.src(avatarbild);
  if(avatar){
    // Den globala bildreserven raknar hela fyrverkeriet som gava. Profilen har egen reserv:
    // initialen under bilden, aldrig en ros eller en annan persons ansikte.
    avatar.hidden=!bild;avatar.removeAttribute('src');
    avatar.onerror=()=>{avatar.hidden=true};
    if(bild)avatar.src=bild;
  }
  const namn=String(avsandare||'Tack för gåvan!');
  e.querySelector('.fw-sender-initial').textContent=Array.from(namn.replace(/^@/,''))[0]?.toUpperCase()||'♥';
  e.querySelector('.fw-sender-name').textContent=namn;
  e.querySelector('.fw-sender-gift').textContent=String(gavonamn||'Gåva')+(combo>1?' × '+combo:'');
  // En ny omgang startar om CSS-tidslinjen aven nar samma DOM-nod redan har .play.
  // Inga per-frame-allokeringar, ingen andra ticker och fortfarande en sluttimer per effekt.
  e.classList.remove('play');
  void e.offsetWidth;
  e.classList.add('play');
  const ms=fwDurationOf(w)*1000,forra=fwTimers.get(e);
...
function buildComboRockets(w,e,combo=w.fwCombo||1,gavobild){
  combo=Math.max(1,Math.min(100,+combo||1));
  e.querySelectorAll('.fw-rocket').forEach(x=>x.remove());
  const burst=e.querySelector('.fw-burst'),gift=VyraSafe.src(gavobild,VyraSafe.src(w.fwGiftImage,FW_GIFT));
  for(let i=0;i<combo;i++){
    const rocket=document.createElement('div');rocket.className='fw-rocket';
    rocket.style.setProperty('--launch-x',`${i===0?0:((i*37)%170)-85}px`);
    rocket.style.setProperty('--delay',`${(i%20)/20*fwFlightOf(w)*.4}s`);
    rocket.style.setProperty('--gift-scale',`${i===0?1:.62+(i%5)*.08}`);
    const img=document.createElement('img');img.className='fw-rocket-gift';img.alt='';img.src=gift;
    img.dataset.fallbackApplied='1';img.onerror=()=>{img.onerror=null;img.src=FW_GIFT};
    rocket.append(img);e.insertBefore(rocket,burst);
  }
  return combo;
}
...
const comboBind=bind;bind=function(){comboBind();let w=liveWidget(selected,'templateGiftFireworks'),test=document.querySelector('#testFw');if(!w||!test||document.querySelector('#fwCombo'))return;let label=document.createElement('label');label.className='fw-combo-control';label.innerHTML=`Antal testraketer<input id="fwCombo" type="number" min="1" max="100" value="${w.fwCombo||1}">`;test.before(label);label.querySelector('input').onchange=e=>{w.fwCombo=Math.max(1,Math.min(100,+e.target.value||1));save()}};
...
const premiumFwProps=props;props=function(){let html=premiumFwProps(),w=liveWidget(selected);if(!w||w.type!=='templateGiftFireworks')return html;let controls=`<div class="property-group fw-premium-motion"><h4>RÖRELSE</h4><div class="fw-motion-options" role="group" aria-label="Rörelsemönster">${[
  ['magnetic','Magnetic Return','En snabb smäll med strålar åt alla håll.','✦'],
  ['spiral','Spiral Recall','En explosion med virvlande gnistor.','◎'],
  ['bloom','Crystal Bloom','En kristallblomma brister ut i ljus.','❋']
].map(([value,label,description,icon])=>`<button type="button" data-fw-choice="${value}" aria-pressed="${(w.fwMotion||'magnetic')===value}"><span class="fw-motion-icon" aria-hidden="true">${icon}</span><span><b>${label}</b><small>${description}</small></span></button>`).join('')}</div><select id="fwMotion" hidden aria-label="Återformning"><option value="magnetic">Magnetic Return</option><option value="spiral">Spiral Recall</option><option value="bloom">Crystal Bloom</option></select><div class="fw-preview"><button type="button" id="testFw">▶ Testa fyrverkeriet</button><small>Gåva → explosion → profilbild och namn.</small></div></div><div class="property-group fw-premium-motion"><h4>ANIMATION · FINJUSTERING</h4><label class="range-label">Återdragningsstyrka <b>${w.fwReturn||80}%</b><input id="fwReturn" type="range" min="40" max="100" value="${w.fwReturn||80}"></label><p>Hur nära mitten partiklarna samlas innan gåvan visas.</p></div><div class="property-group"><h4>BILDER</h4><label>Reservbild för gåva<input id="fwGiftImage" value="${VyraSafe.text(w.fwGiftImage||'assets/gifts/events/0001_Rose.png')}"></label><small>Används vid test och när gåvan saknar bild. Vid live visas den skickade gåvan.</small></div>`;return html.replace('<div class="property-group"><h4>POSITION & STORLEK</h4>',controls+'<div class="property-group"><h4>POSITION & STORLEK</h4>')};
