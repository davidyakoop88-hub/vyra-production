...
function fwSchedule(e){
 ...
 e.querySelectorAll(':scope > .fw-event').forEach(n=>{if(Number(n.dataset.expires)<=Date.now()){window.VyraSupernova?.dispose(n);n.remove()}});
 fwQuality(e);
...
function fwSpela(w,e,combo,gavobild,avatarbild){
 ...
 if(fwCanvas(w)){e.style.animation='none';group.style.animation='none';group.style.translate='none';e.append(group);e.classList.toggle('play',true);window.VyraSupernova.mount(group,w,combo,gavobild,avatarbild,{primary,secondary});fwSchedule(e);return}
...
function fwRenderPreview(e,w){
 if(!e)return;
 if(fwCanvas(w)){const palette=FW_THEMES[fwThemeOf(w)];window.VyraSupernova?.preview(e,w,{primary:bk(w,w.fwColor,'highlight',palette.primary),secondary:bk(w,w.fwColor2,'secondaryText',palette.secondary)});return}
...
window.VyraFireworks={
  ...
  durationFor:input=>Math.max(0,...state.widgets.filter(w=>w.type==='templateGiftFireworks'&&!w.hidden).map(w=>fwSequence(w,fwStyrkaOf(input,w)).duration*1000)),
...
/* GRANSEN OCH TRAPPAN AR TVA OLIKA SIFFROR, och det ar hela poangen.
   Forsta forsoket lat fwMin gora bada jobben: tanda och avgora vad som ar stort. David satte
   fingret pa varfor det inte gar — "den ska funka fran 1 coins gift". Mat pa hans eget band
   fran 2026-09-18:
   ...
   Han vill ha det forsta radens grans och den andra radens trappa. Med en enda siffra gar det
   inte: hojer man den for att gora finalen sallsynt slutar 1-mynts-gavan tanda nagot alls.
   fwMin ar darfor GRANSEN och ingenting annat (fwSlapperIgenom). fwFinal ar vad en STOR FINAL
   kostar, och firandet ligger en tiondel dar — en siffra att stalla in, inte tva, och de kan
   inte hamna i fel ordning.
   RAKNAS PER WIDGET. Flera fyrverkerier med olika instalIningar ar en avsiktlig funktion (se
   kommentaren vid traffar nedan), sa styrkan kan inte raknas en gang for alla — samma gava kan
   vara en final for den ena widgeten och en vanlig tandning for den andra. */
const FW_FINAL=1000;
const fwFinalOf=w=>{const n=Number(w&&w.fwFinal);return Number.isFinite(n)&&n>0?n:FW_FINAL};
const fwStyrkaOf=(input,w)=>{const d=input&&typeof input==='object'?input:{combo:input};
 const varde=fwTotalOf(d);
 if(!Number.isFinite(varde))return fwComboOf(d);
 const final=fwFinalOf(w);
 return varde>=final?100:varde>=final/10?10:1};
...
/* Styrkan raknas per widget langre ned, inte har: tva fyrverkerier med olika fwMin har olika
   trappa, och samma gava kan darfor vara final for det ena och en vanlig tandning for det andra. */
...
traffar.forEach(w=>{const e=document.querySelector(`[data-id="${w.id}"] .gift-fireworks-fx`);if(!e)return;/* textContent, aldrig innerHTML: anvandarnamnet kommer fran TikTok via molnet och ar inte
   betrott innehall. */
const tx=e.querySelector('.fw-text');if(tx)tx.textContent=String(w.fwText||'{user} skickade {gift}').split('{user}').join(d.username||d.name||'').split('{gift}').join(d.giftName||d.gift||'');fwSpela(w,e,fwStyrkaOf(d,w),d.giftImage,d.profileImage||d.profileUrl,d.username||d.name,d.giftName||d.gift);nagotTandes=true});
...
document.addEventListener('click',event=>{if(!event.target.closest('[data-fw] button'))return;setTimeout(()=>{let w=state.widgets.filter(x=>x.type==='templateGiftFireworks').at(-1);if(!w||fwCanvas(w))return;w.x=120;w.y=380;w.width=360;w.hidden=false;w.fwGiftImage=campaignGiftList()[0]?.file;selected=w.id;save();render()},0)});
