(() => {
  ...
  function enhance(){
    ...
    save.onclick=async()=>{
      ...
      try{const state=JSON.parse(localStorage.getItem(KEY)||'{"actions":[],"events":[]}');state.actions.push({id:'a'+Date.now(),name,types,duration:+modal.querySelector('#aeDuration').value,cooldown:+modal.querySelector('#aeCooldown').value,volume:+modal.querySelector('#aeVolume').value,fade:modal.querySelector('#aeFade').checked,repeatCombo:modal.querySelector('#aeRepeatCombo').checked,pictureMedia:await store(picture),videoMedia:video?await store(video):(selectedPackageVideo?{packagePath:selectedPackageVideo.path,name:selectedPackageVideo.label}:null),audioMedia:await store(audio)});localStorage.setItem(KEY,JSON.stringify(state));modal.remove();window.toast?.('Action och media sparades');document.querySelector('[data-extra=actions]')?.click()}
