(() => {
  ...
  function enhance(){
    ...
    save.onclick=async()=>{
      ...
      try{const state=JSON.parse(localStorage.getItem(KEY)||'{"actions":[],"events":[]}');state.actions.push({id:'a'+Date.now(),name,types,duration:+modal.querySelector('#aeDuration').value,cooldown:+modal.querySelector('#aeCooldown').value,userCooldown:+modal.querySelector('#aeUserCooldown').value,volume:+modal.querySelector('#aeVolume').value,fade:modal.querySelector('#aeFade').checked,repeatCombo:modal.querySelector('#aeRepeatCombo').checked,skipOnNext:modal.querySelector('#aeSkipOnNext').checked,pictureMedia:await store(picture),videoMedia:video?await store(video):(selectedPackageVideo?{packagePath:selectedPackageVideo.path,name:selectedPackageVideo.label}:null),audioMedia:await store(audio)});window.VyraSessionState.writeActive(KEY,JSON.stringify(state));modal.remove();window.toast?.('Action och media sparades');document.querySelector('[data-extra=actions]')?.click()}
