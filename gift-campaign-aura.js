...
const oldProps=props;props=function(){let result=oldProps(),w=liveWidget(selected);if(!w||w.type!=='templateGiftCampaign')return result;
 result=result.replace(/<select id="campaignTheme">[\s\S]*?<\/select>/,`<select id="campaignTheme">${themes.map(([id,label])=>`<option value="${id}" ${theme(w)===id?'selected':''}>${label}</option>`).join('')}</select>`).replace(/(<select id="campaignOrientation">[\s\S]*?<option value="landscape">)[^<]*/,'$1Liggande · gåvor i rad').replace(/(<select id="campaignOrientation">[\s\S]*?<option value="portrait">)[^<]*/,'$1Stående · gåvor i följd');
