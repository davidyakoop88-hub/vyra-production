...
: "${VYRA_TIKTOK_BRIDGE_IMAGE:?Set immutable VYRA_TIKTOK_BRIDGE_IMAGE (prefer digest)}"
...
case "$VYRA_API_IMAGE $VYRA_WEB_IMAGE $VYRA_TIKTOK_BRIDGE_IMAGE" in *:latest*) echo "latest image tags are forbidden" >&2; exit 1;; esac
node scripts/production-preflight.js
mkdir -p .deploy
if [ -f .deploy/current-images ]; then cp .deploy/current-images .deploy/previous-images; fi
printf 'VYRA_API_IMAGE=%q\nVYRA_WEB_IMAGE=%q\nVYRA_TIKTOK_BRIDGE_IMAGE=%q\n' "$VYRA_API_IMAGE" "$VYRA_WEB_IMAGE" "$VYRA_TIKTOK_BRIDGE_IMAGE" > .deploy/current-images
...
docker compose -f docker-compose.production.yml up -d --no-deps tiktok-connection-manager
