(() => {
  ...
  function shell(){const state=read();return `<div class="ae-head"><div><h2>Action &amp; Event</h2><p>Skapa en Action först. Koppla sedan ett Event som triggar den.</p></div></div><div class="ae-steps"><b>1</b><span>Skapa Action</span><i>→</i><b>2</b><span>Koppla Event</span><i>→</i><b>3</b><span>Trigger kör Action</span></div><div class="ae-columns"><section class="card ae-actions-card"><header><h3>Actions</h3><span>${state.actions.length} sparade</span></header><p class="ae-card-intro">Vad vill du ska hända? Här skapar du nya actions och redigerar dem som redan finns. Längre ner kopplar du dem till ett event.</p><div class="ae-actions-toolbar"><button id="newAeAction" class="primary">＋ Skapa ny Action</button>${state.actions.length?`<label class="ae-check"><input id="aeMasterEnabled" type="checkbox"${actionsEnabled(state)?' checked':''}> Aktiverad</label><input id="aeActionSearch" type="search" placeholder="Sök bland dina actions…">`:''}</div>${state.actions.length?`<div class="ae-actions-scroll"><table class="ae-actions-table"><thead><tr><th></th><th>Namn</th><th>Skärm</th><th>Visningstid (sek.)</th><th>Poäng +/−</th>${listColumnTypes.map(x=>`<th class="ae-col-check">${x[1]}</th>`).join('')}<th>Beskrivning</th></tr></thead><tbody>${state.actions.map(actionRow).join('')}</tbody></table><p class="ae-sok-tomt" hidden>Ingen action matchar sökningen.</p></div>`:'<p data-tom="automatik-actions">Inga Actions ännu. Skapa den första.</p>'}</section><section class="card ae-events-card"><header><h3>Events</h3><span>${state.events.length} kopplingar</span></header><p class="ae-card-intro">Här bestämmer du vad som ska trigga dina actions.</p><div class="ae-actions-toolbar"><button id="newAeEventCard" class="primary">＋ Skapa nytt Event</button>${state.events.length?`<input id="aeEventSearch" type="search" placeholder="Sök bland dina events…">`:''}</div>${state.events.length?`<div class="ae-actions-scroll"><table class="ae-actions-table ae-events-table"><thead><tr><th></th><th class="ae-col-check">Aktiv</th><th>Användare</th><th>Trigger</th><th>Action(er)</th></tr></thead><tbody>${state.events.map(e=>eventRow(state,e)).join('')}</tbody></table><p class="ae-sok-tomt" data-tomt="event" hidden>Inget event matchar sökningen.</p></div>`:'<p data-tom="automatik-events">Inga Events ännu. Koppla ett event till en Action.</p>'}</section></div><div id="aeModal"></div>`}
  ...
  function bindPage(){document.querySelector('#newAeAction').onclick=()=>actionModal();const nyttEvent=()=>eventModal();document.querySelectorAll('#newAeEvent,#newAeEventCard').forEach(b=>b.onclick=nyttEvent);
