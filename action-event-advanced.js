(() => {
  ...
  function enhance(){
    ...
    const s=JSON.parse(localStorage.getItem(KEY)||'{"actions":[],"events":[]}'),footer=m.querySelector('footer');
    ...
    const before=s.events.length;m.querySelector('#saveAeEvent').addEventListener('click',()=>{const selected=q=>[...p.querySelector(q).selectedOptions].map(o=>o.value),extra={audience:p.querySelector('[name=aeAudience]:checked').value,specificUser:p.querySelector('#aeSpecificUser').value.trim().replace(/^@/,''),advancedTrigger:p.querySelector('[name=aeAdvancedTrigger]:checked').value,triggerValue:p.querySelector('#aeAdvancedValue')?.value?.trim()||'',teamLevel:+p.querySelector('#aeTeamLevel').value,allActionIds:selected('#aeAllActions'),randomActionIds:selected('#aeRandomActions')};if(extra.allActionIds.length)m.querySelector('#aeActionId').value=extra.allActionIds[0];else if(extra.randomActionIds.length)m.querySelector('#aeActionId').value=extra.randomActionIds[0];let i=0,t=setInterval(()=>{const state=JSON.parse(localStorage.getItem(KEY)||'{"events":[]}');if(state.events.length>before){Object.assign(state.events.at(-1),extra);localStorage.setItem(KEY,JSON.stringify(state));clearInterval(t)}else if(++i>40)clearInterval(t)},100)},true);
  }
