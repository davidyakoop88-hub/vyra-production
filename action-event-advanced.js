(() => {
  ...
  function allowed(e,p={}){if((p.teamLevel||0)<(e.teamLevel||0))return false;if(e.excludeAnonymous&&p.isAnonymous)return false;if(!e.audience||e.audience==='everyone')return true;if(e.audience==='specificUser')return String(p.username||p.user||'').replace(/^@/,'').toLowerCase()===String(e.specificUser||'').toLowerCase();return e.audience===p.role||(e.audience==='follower'&&p.isFollower)||(e.audience==='subscriber'&&p.isSubscriber)||(e.audience==='moderator'&&p.isModerator)||(e.audience==='topGifter'&&p.isTopGifter)}
