'use strict';
// Deterministiska fasfoton for Rise & Pop (profile) (arbetsgangens steg 7).
...
(async () => {
  ...
  // inte pa minuter; sekvensen ar 1220 ms och fotona tas i flykten.
  await page.evaluate(() => window.triggerGifterLevelUp({ __test: true, level: 12, name: 'RisePop', username: 'prov' }));
  const foton = [['fas1-glimt', 200], ['fas2-stigning', 620], ['fas3-pop', 1000], ['efter-vila', 1550]];
  let forra = 0;
  for (const [namn, vid] of foton) {
    await page.waitForTimeout(vid - forra); forra = vid;
    await page.screenshot({ path: path.join(__dirname, 'rise-pop-' + namn + '.png'), clip: klipp });
  }
