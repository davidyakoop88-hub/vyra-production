...
- `vyra-state-sync.js` – exporterar och importerar hela Studio-state (localStorage) som en JSON-fil, så att scener, widgets och Action & Event kan flyttas mellan datorer. Fristående och laddad sist i `studio.html`. Ritar inget i overlay-läge (`?overlay=1` eller `?access=`), och maskin-/workspace-bundna nycklar följer aldrig med i exporten.
